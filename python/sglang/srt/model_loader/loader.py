...
class RemoteInstanceModelLoader(BaseModelLoader):
        ...
        *,
        ...
        if (
            ...
            try:
                ...
                # Opt-in on success (the registration is dead weight, but it is
                # charged against the KV pool); unconditional on failure, so
                # the disk fallback has the memory to load into.
                released = True
                if (
                    success is not True
    ...
    def _fallback_to_disk(
        ...
        *,
        ...
        try:
            fallback_format = (
                LoadFormat(configured) if configured else LoadFormat.AUTO
            )
        except ValueError:
            logger.warning(
    ...
    if load_config.load_format == LoadFormat.IPC_CACHE:
        ...
        if load_config.weight_cache_socket:
            ...
            socket_path = get_socket_path(
                ...
                is_draft_model=load_config.weight_cache_is_draft,
            )
