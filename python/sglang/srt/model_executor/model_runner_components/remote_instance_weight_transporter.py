...
class RemoteInstanceWeightTransporter:
    ...
    def maybe_register_and_publish_weight_info(self) -> None:
        if (
            remote_instance_transfer_engine_enabled()
            # The draft shares the target's tp_rank, which is the bootstrap
            # server's key: publishing here would replace the target's manifest
            # with the draft's, and a client asking for target weights would
            # find none of them.
